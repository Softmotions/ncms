...
.. glossary::
    ...
    атрибут
    attribute
        Именованный блок данных, принадлежащий сборке. Это может быть простая строка, либо
    ...
    ядро
    core
        Ядро сборки это HTTL разметка для представления данных `сборки` в виде HTML
...
Редактор сайта, используя nCMS GUI может создать экземпляр страницы с именем `mypage`,
...
:term:`сборки <сборка>` `mypage` и выполнит генерацию HTML клиенту. В этом примере
описана основная идея, лежащая в nCMS.

Реализация примера в nCMS
*************************
